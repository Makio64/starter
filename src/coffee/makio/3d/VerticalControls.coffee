#
# VerticalControls using theta / y / radius
# @author David Ronai / http://makiopolis.com / @makio64
#

Stage3d = require("core/Stage3d")
Interactions = require('core/Interactions')

class VerticalControls

	constructor:(@camera, @radius=300)->
		if(!@camera)
			console.warn('VerticalControls need a camera as parameter')
	...
	initGui:(gui)->
		f = gui.addFolder('VerticalControls')
	...
	refreshAngle:()->
		...
		@target.y = @camera.position.y - @offset.y
...
module.exports = VerticalControls
